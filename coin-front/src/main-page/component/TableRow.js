...
const TableRow = ({ id, img_link, ticker, name, price_binance, price_upbit, price_gap, gap_percent }) => {
  const icon = gap_percent >= 0 ? faCaretUp : faCaretDown;
  const gapClass = gap_percent >= 0 ? 'positive-gap' : 'negative-gap';
  const formattedGapPercent = gap_percent >= 0 ? gap_percent.toFixed(2) : -gap_percent.toFixed(2);
  ...
  return (
    <div className="coin-table-row">
      <div className='cells'>
        ...
        <div className={`cell ${gapClass}`}>{price_gap} <FontAwesomeIcon icon={icon} /></div>
